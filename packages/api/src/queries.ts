...
export const pokemonDetailQuery = (idOrName: string | number) => queryOptions({
  queryKey: pokemonKeys.detail(idOrName),
  queryFn: async ({ signal }) => mapPokemonDetail(await apiGet<PokemonApiResponse>(`/pokemon/${idOrName}`, signal)),
  staleTime: 10 * 60 * 1000,
  retry: (count, error) => !(error instanceof Error && 'status' in error && error.status === 404) && count < 2,
});

export const pokemonTypeQuery = (type: string) => queryOptions({
...
export const pokemonListInfiniteQuery = () => infiniteQueryOptions({
  queryKey: pokemonKeys.list(30),
  initialPageParam: 0,
  queryFn: async ({ pageParam, signal }) => {
    const data = await apiGet<PokemonListResponse>(`/pokemon?limit=30&offset=${pageParam}`, signal);
    return { items: data.results.map((resource) => mapPokemonResource(resource)), nextOffset: data.next ? pageParam + 30 : null };
  },
