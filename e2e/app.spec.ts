...
const pokemon = {
  ...
  sprites: {
    front_default: 'data:image/gif;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs=',
    other: {
      dream_world: { front_default: 'data:image/gif;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs=' },
    },
  },
};
...
async function mockApi(page: Page) {
  await page.route('https://pokeapi.co/api/v2/**', async (route) => {
    ...
    if (url.pathname.startsWith('/api/v2/type/')) {
      await route.fulfill({
        json: {
          pokemon: Array.from({ length: 10 }, (_, index) => ({
            slot: index + 1,
            pokemon: {
              name: index === 0 ? 'pikachu' : `pokemon-${index + 1}`,
              url: `https://pokeapi.co/api/v2/pokemon/${index + 25}/`,
            },
          })),
        },
      });
      ...
      await route.fulfill({
        json: {
          ...
          results: Array.from({ length: 30 }, (_, index) => ({
            name: index === 0 ? 'pikachu' : `pokemon-${offset + index + 1}`,
            url: `https://pokeapi.co/api/v2/pokemon/${offset + index + 25}/`,
          })),
        },
      });
      return;
    }
    if (url.pathname.endsWith('/pikachu') || url.pathname.endsWith('/25')) {
      await route.fulfill({ json: pokemon });
      return;
    }
...
test('login → detail remote → persistent history', async ({ page }) => {
  await login(page);
  await expect(page.getByRole('heading', { name: /Explora Pokémon/ })).toBeVisible();
...
test('toast acknowledgement remains stable until a new visit', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'pikachu' })).toBeVisible();
  await page.reload();
  await expect(page.getByText('Última visita', { exact: true })).toBeVisible();
  await page.getByRole('button', { name: 'Cerrar notificación' }).click();
  await page.reload();
  await expect(page.getByText('Última visita', { exact: true })).toHaveCount(0);
  await expect(page.getByRole('heading', { name: 'pikachu' })).toBeVisible();
  await page.goto('/');
  await page.getByRole('button', { name: 'Ver detalle de pikachu' }).first().click();
  await expect(page.getByRole('heading', { name: 'pikachu' })).toBeVisible();
...
test('accessible exact search normalizes accents', async ({ page }) => {
  ...
  await input.fill('  PÍKACHU  ');
  await page.getByRole('dialog').getByRole('button', { name: 'Buscar', exact: true }).click();
  await expect(
    page.getByRole('dialog').getByRole('button', { name: 'Ver detalle de pikachu' }),
  ).toBeVisible();
